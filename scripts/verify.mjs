...
async function checkRedirects() {
  ...
  // Per-album rules are generated by generate_photo_redirects.mjs.
  // Verify at least one hardcoded-destination photo rewrite rule is present.
  if (!/\/i-:key\s+.*\/index\.html\s+200/.test(text)) {
    failures.push('_redirects missing generated per-album photo rewrite rules (run build to generate)');
  }
  return { name: '_redirects present', failures };
}
