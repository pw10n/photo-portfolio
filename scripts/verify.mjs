...
async function checkRedirects() {
  const failures = [];
  // Photo URL routing is now handled by functions/[[path]].js.
  // Verify the function file exists in the repo (it is deployed alongside dist/).
  const fnPath = resolve(REPO_ROOT, 'functions', '[[path]].js');
  if (!(await pathExists(fnPath))) {
    failures.push('functions/[[path]].js missing — photo URL routing will not work');
  }
